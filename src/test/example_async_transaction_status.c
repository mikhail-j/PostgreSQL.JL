/*
* This is an example of parsing asynchronously executed SQL commands and checking SSL and transaction status.
*
...
void postgres_protocol_version (PGconn* pgc) {
	int p_version = PQprotocolVersion(pgc);
	
	if (p_version != 0) {	//is this connection even properly established?
		printf("postgresql protocol version: %i.0\n", p_version);
	}
	else {
		printf("error: no valid libpq server connection found!\n");
	}
...
int main (int argc, const char* argv[]) {
	...
	if (argc > 1) {		//check if a filename was given
		...
		if (param_count != -1) {
			...
			postgres_protocol_version(postgres_nb_param_conn);
			postgres_ssl_enabled(postgres_nb_param_conn);
			
			//run SQL statement with PQsendQuery(), values are returned in text form
			PGresult* first_sql_command;
			int isSent;
			
			isSent = PQsendQuery(postgres_nb_param_conn, "SELECT * FROM example_balance;");
			if (isSent) {
				printf("\nSQL statement was asynchronously sent with PQsendQuery()!\n");
				
				uint64_t cycles = 0;
				while (PQisBusy(postgres_nb_param_conn)) {		//do nothing until result is found
					PQconsumeInput(postgres_nb_param_conn);
					++cycles;
				}
				
				printf("number of cycles before result was found: %lli\n", cycles);
				first_sql_command = PQgetResult(postgres_nb_param_conn);
				
				while (PQgetResult(postgres_nb_param_conn) != NULL) {}				//following queries will get error otherwise, perhaps we should consider assigning non-NULL results to a linked list
				...
				printf("error: SQL statement could not be sent with PQsendQuery().\n");
				printf("libpq error: %s\n", PQerrorMessage(postgres_nb_param_conn));
			}

			//run SQL statement with PQsendQueryParams(), specifying binary format results
			PGresult* second_sql_command;
			isSent = PQsendQueryParams(postgres_nb_param_conn, "SELECT * FROM example_balance;", 0, NULL, NULL, NULL, NULL, 1);
			if (isSent) {
				printf("\nSQL statement was asynchronously sent with PQsendQueryParams()!\n");
				...
				second_sql_command = PQgetResult(postgres_nb_param_conn);
				...
				printf("error: SQL statement could not be sent with PQsendQueryParams().\n");
				printf("libpq error: %s\n", PQerrorMessage(postgres_nb_param_conn));
			}
			
/*
 * PQsendPrepare() and PQsendQueryPrepared() do not work unless libpq protocol version is at least 3.0 (https://www.postgresql.org/docs/9.5/static/libpq-async.html).
 *
 */
			
			PGresult* third_sql_command_text;
			PGresult* third_sql_command_binary;
			
			isSent = PQsendPrepare(postgres_nb_param_conn, "third", "SELECT * FROM example_balance;", 0, NULL);
			if (isSent) {
				printf("\nSQL statement was asynchronously sent with PQsendPrepare()!\n");
				...
				while (PQgetResult(postgres_nb_param_conn) != NULL) {}		//PQgetResult() must return NULL before successfully sending another query

				//specify text format results
				isSent = PQsendQueryPrepared(postgres_nb_param_conn, "third", 0, NULL, NULL, NULL, 0);		//text as specified by resultFormat
				if (isSent) {
					printf("\nSQL statement was asynchronously sent with PQsendQueryPrepared()!\n");
					...
					third_sql_command_text = PQgetResult(postgres_nb_param_conn);
					
					while (PQgetResult(postgres_nb_param_conn) != NULL) {}				//following queries will get error otherwise, perhaps we should consider assigning non-NULL results to a linked list
					
					postgres_result_status(third_sql_command_text);
					printf("SQL command executed: %s\n", PQcmdStatus(third_sql_command_text));
					printf("number of rows affected by SQL command: %s\n", PQcmdTuples(third_sql_command_text));
					printf("number of rows returned: %i | number of columns returned: %i\n", PQntuples(third_sql_command_text), PQnfields(third_sql_command_text));
					//Oid column_zero = PQftype(third_sql_command_text, 0);
					//Oid column_one = PQftype(third_sql_command_text, 1);
					//Oid column_two = PQftype(third_sql_command_text, 2);
					printf("column 1 oid: %u | column 2 oid: %u | column 3 oid: %u\n", PQftype(third_sql_command_text, 0), PQftype(third_sql_command_text, 1), PQftype(third_sql_command_text, 2));
					printf("column 1 fmt_code: %i | column 2 fmt_code: %i | column 3 fmt_code: %i\n", PQfformat(third_sql_command_text, 0), PQfformat(third_sql_command_text, 1), PQfformat(third_sql_command_text, 2));
					printf("column names: %s %s %s\n", PQfname(third_sql_command_text, 0), PQfname(third_sql_command_text, 1), PQfname(third_sql_command_text, 2));
					printf("row 1: %s %s %s\n", PQgetvalue(third_sql_command_text, 0, 0), PQgetvalue(third_sql_command_text, 0, 1), PQgetvalue(third_sql_command_text, 0, 2));
					printf("row 2: %s %s %s\n", PQgetvalue(third_sql_command_text, 1, 0), PQgetvalue(third_sql_command_text, 1, 1), PQgetvalue(third_sql_command_text, 1, 2));
					printf("row 3: %s %s %s\n", PQgetvalue(third_sql_command_text, 2, 0), PQgetvalue(third_sql_command_text, 2, 1), PQgetvalue(third_sql_command_text, 2, 2));
					
					//clean up PGresult objects
					PQclear(third_sql_command_text);
				}
				else {
					printf("error: SQL statement could not be sent with PQsendQuery().\n");
					printf("libpq error: %s\n", PQerrorMessage(postgres_nb_param_conn));
				...
				//specify binary format results
				isSent = PQsendQueryPrepared(postgres_nb_param_conn, "third", 0, NULL, NULL, NULL, 1);		//binary as specified by resultFormat
				if (isSent) {
					printf("\nSQL statement was asynchronously sent with PQsendQueryPrepared()!\n");
					...
					third_sql_command_binary = PQgetResult(postgres_nb_param_conn);
					
					while (PQgetResult(postgres_nb_param_conn) != NULL) {}				//following queries will get error otherwise, perhaps we should consider assigning non-NULL results to a linked list
					
					postgres_result_status(third_sql_command_binary);
					printf("SQL command executed: %s\n", PQcmdStatus(third_sql_command_binary));
					printf("number of rows affected by SQL command: %s\n", PQcmdTuples(third_sql_command_binary));
					printf("number of rows returned: %i | number of columns returned: %i\n", PQntuples(third_sql_command_binary), PQnfields(third_sql_command_binary));
					//Oid column_zero = PQftype(third_sql_command_binary, 0);
					//Oid column_one = PQftype(third_sql_command_binary, 1);
					//Oid column_two = PQftype(third_sql_command_binary, 2);
					printf("column 1 oid: %u | column 2 oid: %u | column 3 oid: %u\n", PQftype(third_sql_command_binary, 0), PQftype(third_sql_command_binary, 1), PQftype(third_sql_command_binary, 2));
					printf("column 1 format code: %i | column 2 format code: %i | column 3 format code: %i\n", PQfformat(third_sql_command_binary, 0), PQfformat(third_sql_command_binary, 1), PQfformat(third_sql_command_binary, 2));
					printf("column names: %s %s %s\n", PQfname(third_sql_command_binary, 0), PQfname(third_sql_command_binary, 1), PQfname(third_sql_command_binary, 2));
					
					int64_t* parsebigint = malloc(sizeof(char) * 8);
					double* parsedouble = malloc(sizeof(char) * 8);
					int nrows = PQntuples(third_sql_command_binary);
					int ncols = PQnfields(third_sql_command_binary);
					
					for (int i = 0; i < nrows; i++) {
						memcpy(parsebigint, PQgetvalue(third_sql_command_binary, i, 0), 8);
						parsebigint[0] = __builtin_bswap64(*(parsebigint));		//use gcc's builtin 64 bit byte swap function
						
						memcpy(parsedouble, PQgetvalue(third_sql_command_binary, i, 1), 8);
						bswap_double(parsedouble);
						
						printf("row %i: %lli %f %s\n", i + 1, parsebigint[0], parsedouble[0], PQgetvalue(third_sql_command_binary, i, 2));
					}
					...
					//clean up PGresult objects
					PQclear(third_sql_command_binary);
				}
				else {
					printf("error: SQL statement could not be sent with PQsendQueryParams().\n");
					printf("libpq error: %s\n", PQerrorMessage(postgres_nb_param_conn));
				...
				if (isSent) {
					printf("SQL statement was asynchronously sent!\n");
					...
					printf("error: SQL statement could not be sent with PQsendQueryPrepared().\n");
					printf("libpq error: %s\n", PQerrorMessage(postgres_nb_param_conn));
				}
				
			}
			else {
				printf("error: SQL statement could not be sent with PQsendPrepare().\n");
				printf("libpq error: %s\n", PQerrorMessage(postgres_nb_param_conn));
			}
