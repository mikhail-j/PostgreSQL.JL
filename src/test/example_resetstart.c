/*
* This is an example of using PQresetStart() (a non-blocking connection to the database).
*
...
int main (int argc, const char* argv[]) {
	...
	//reset non-blocking connection
	int reset_success = PQresetStart(postgres_conn);
	
	//check if non-blocking connection is usable
	if (reset_success == 1) {
		printf("libpq: Resetting non-blocking connection!\n");
		int reset_poll_status = PQresetPoll(postgres_conn);
		while (reset_poll_status != PGRES_POLLING_OK && reset_poll_status != PGRES_POLLING_FAILED) {
			reset_poll_status = PQresetPoll(postgres_conn);
		}
	}
	else {
		printf("error: non-blocking connection could not be reset!\n");
	}
